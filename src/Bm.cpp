...
namespace cv
{
	...
	/*
struct BmPrefilterInvoker
{
    BmPrefilterInvoker(const cv::Mat& left0, const cv::Mat& right0, cv::Mat& left, cv::Mat& right,
    ...
    void operator()( int ind ) const
    {
        if( state->preFilterType == CV_STEREO_BM_NORMALIZED_RESPONSE )
            Bm_prefilterNorm( *imgs0[ind], *imgs[ind], state->preFilterSize, state->preFilterCap, buf[ind] );
        else
            Bm_prefilterXSobel( *imgs0[ind], *imgs[ind], state->preFilterCap );

    }

	

    const cv::Mat* imgs0[2];
    cv::Mat* imgs[2];
    uchar* buf[2];
...
struct BmFindStereoCorrespInvoker : cv::ParallelLoopBody
{
    BmFindStereoCorrespInvoker( const cv::Mat& _left, const cv::Mat& _right,
                              cv::Mat& _disp,CvStereoBMState* _state,
                              int _nstripes, int _stripeBufSize,
                              bool _useShorts, cv::Rect _validDisparityRect )
    {
    ...
    {
		//std::cout<<range.start <<"..."<<std::endl;                              // this progress parallel do //
        int cols = left->cols, rows = left->rows;
        int _row0 = cv::min(cvRound(range.start * rows / nstripes), rows);
        int _row1 = cv::min(cvRound(range.end * rows / nstripes), rows);
        uchar *ptr = state->slidingSumBuf->data.ptr + range.start * stripeBufSize;     // start address ip 
        int FILTERED = (state->minDisparity - 1)*16;

        cv::Rect roi = validDisparityRect & cv::Rect(0, _row0, cols, _row1 - _row0);
        if( roi.height == 0 )
            return;
        int row0 = roi.y;
        int row1 = roi.y + roi.height;

        cv::Mat part;                                                     //////////////  What is part used for  ??????  ///////////////////
        if( row0 > _row0 ) 
        {
            part = disp->rowRange(_row0, row0);
            part = cv::Scalar::all(FILTERED);
        }
        if( _row1 > row1 )
        {
        ...
        cv::Mat left_i = left->rowRange(row0, row1);                 //   left_i for one stripe              cut roi, cut row only, do not cut column
        cv::Mat right_i = right->rowRange(row0, row1);               //   right_i for one stripe             cut roi, cut row only, do not cut column

		//disp->copyTo(*diff);

        cv::Mat disp_i = disp->rowRange(row0, row1);
		//cv::Mat diff_i = diff->rowRange(row0, row1);


        cv::Mat cost_i = state->disp12MaxDiff >= 0 ? cv::Mat(state->cost).rowRange(row0, row1) : Mat();     //////////// if disp12MaxDiff<0  cost_i is empty


#if CV_SSE2
        if( useShorts )
            Bm_findStereoCorrespondenceBM_SSE2( left_i, right_i, disp_i, cost_i, *state, ptr, row0, rows - row1 );
        else
#endif
            Bm_findStereoCorrespondenceBM( left_i, right_i, disp_i, cost_i, *state, ptr, row0, rows - row1 );    // add diff_i as output

		double minVal;
        double maxVal;
        cv::minMaxLoc(disp_i, &minVal, &maxVal);

		cv::minMaxLoc(cost_i, &minVal, &maxVal);
...
protected:
    ...
    int stripeBufSize;
    bool useShorts;
    cv::Rect validDisparityRect;
};



/*
void Bm_prefilterNorm( const  Mat& src,  Mat& dst, int winsize, int ftzero, uchar* buf )
{
    int x, y, wsz2 = winsize/2;
    int* vsum = (int*) alignPtr(buf + (wsz2 + 1)*sizeof(vsum[0]), 32);   // sizeof(vsum[0]), size of int,4 byte;   sizeof: how many byte of int; 1 byte = 8 bit, int = 32 bit;

    int scale_g = winsize*winsize/8, scale_s = (1024 + scale_g)/(scale_g*2);  // assume there is 8 marker in one windows
    ...
    for( x = 0; x < TABSZ; x++ )
	{
        tab[x] = (uchar)(x - OFS < -ftzero ? 0 : x - OFS > ftzero ? ftzero*2 : x - OFS + ftzero);              // uchar �ַ�  
	}
...
void
Bm_prefilterXSobel( const  Mat& src,  Mat& dst, int ftzero )
{
    int x, y;
...
#if CV_SSE2
void Bm_findStereoCorrespondenceBM_SSE2( const  Mat& left, const  Mat& right,
...
void
Bm_findStereoCorrespondenceBM( const  Mat& left, const  Mat& right,                     //////////// done
...
void Bm_findStereoCorrespondenceBM( const Mat& left0, const Mat& right0, Mat& disp0, CvStereoBMState* state,int nstripes,bool useShorts)
{
    ...
    if( !state->preFilteredImg0 || state->preFilteredImg0->cols * state->preFilteredImg0->rows < left0.cols * left0.rows )
    {
     ...
     //Mat left(left0.size(), CV_8U, state->preFilteredImg0->data.ptr);     // initialize a matrix have the same size of input(left) image matrix
     //Mat right(right0.size(), CV_8U, state->preFilteredImg1->data.ptr);   // initialize a matrix have the same size of input(right) image matrix

    int mindisp = state->minDisparity;
    int ndisp = state->numberOfDisparities;     // ndisp = 16*n (int n)

    int width = left0.cols;
    int height = left0.rows;
    int lofs =  max(ndisp - 1 + mindisp, 0);
    int rofs = - min(ndisp - 1 + mindisp, 0);
    int width1 = width - rofs - ndisp + 1;      // output depth image width
    int FILTERED = (state->minDisparity - 1) << DISPARITY_SHIFT;    //start buffer positon
    ...
    if( disp0.type() == CV_32F)
    {
        ...
        disp =  cvarrToMat(state->disp);    // transfer array to  mat
    }
	//std::cout<<disp<<std::endl;

    int wsz = state->SADWindowSize;
    int bufSize0 = (int)((ndisp + 2)*sizeof(int));  // sizeof: how many byte of int; 1 byte = 8 bit, int = 32 bit;  +2 for blur matching?;   bufSize0, how many byte of buffer
    bufSize0 += (int)((height+wsz+2)*ndisp*sizeof(int));
    bufSize0 += (int)((height + wsz + 2)*sizeof(int));
	//std::cout<<sizeof(uchar)<<std::endl;
    bufSize0 += (int)((height+wsz+2)*ndisp*(wsz+2)*sizeof(uchar) + 256);  // unchar 1 byte, sizeof(uchar) = 1
	...
	//bool useShorts = 1;
	//int nstripes = 1;                               ///////////////////////////////////////////////////////////  nstripes = 1 for debug mode, should use the cpu core number ////////
    int bufSize =  bufSize0 * nstripes;
    ...
    //int idx[] = {0,1};
    //parallel_do(idx, idx+2, BmPrefilterInvoker(left0, right0, left, right, _buf, _buf + bufSize1, state));    // buffer for compute SAD ,bufSize1 for compute prefilter
    ...
    validDisparityRect =  getValidDisparityROI(R1.area() > 0 ?  R1 : validDisparityRect,
                                              ...
                                              state->SADWindowSize);
	//std::cout<<validDisparityRect<<std::endl;
	cv::Mat diff;
	disp.copyTo(diff);
    ...
    parallel_for_( Range(0, nstripes),
                  BmFindStereoCorrespInvoker(left0, right0, disp, state, nstripes,   ////   loop for 0 : nstripes
	...
	//diff.convertTo(diff0, disp0.type(), 1);
	//diff0 = diff.clone();
...
CV_WRAP  MyBM::
	MyBM():StereoBM()
{
	std::cout<<"have initialized!!"<<std::endl;
}

CV_WRAP MyBM::
	MyBM(int preset, int ndisparities, int SADWindowSize) :StereoBM(preset, ndisparities, SADWindowSize)
{
	std::cout<<"have initialized!!"<<std::endl;
}


 void MyBM::operator()( cv::InputArray _left, cv::InputArray _right,
                           cv::OutputArray _disparity, int disptype ,int nstripes,bool useShorts)
	{	
    cv::Mat left = _left.getMat(), right = _right.getMat();			/// copy

    CV_Assert( disptype == CV_16S || disptype == CV_32F );		/// CV_Assert: if false error
    _disparity.create(left.size(), disptype);

    cv::Mat diff;
    cv::Mat disparity = _disparity.getMat();

    cv::Bm_findStereoCorrespondenceBM(left, right, disparity, state,nstripes,useShorts);//state : pointer  ??????
